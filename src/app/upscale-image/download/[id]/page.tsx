...
import { resetUpscale } from "~/redux/slices/upscaleSlice";
...
const DownloadPage = () => {
  ...
  const { upscaledFiles } = useAppSelector((s) => s.upscale);
  const handleDownloadFile = async () => {
    if (!upscaledFiles?.length) return;

    if (upscaledFiles.length === 1) {
      const url = upscaledFiles?.[0]?.upscaledUrl ?? "";
      const name = filenameFromUrl(url, 0, { replaceTokenWith: "upscaled" });
      try {
        await downloadOne(url, name);
        await deleteImage(upscaledFiles?.[0]?.fileId ?? "");
      } catch (e) {
    ...
    for (let i = 0; i < upscaledFiles.length; i++) {
      const url = upscaledFiles?.[i]?.upscaledUrl ?? "";
      const name = filenameFromUrl(url, i, { replaceTokenWith: "upscaled" });
      try {
        const res = await fetch(url, { credentials: "omit" });
        if (!res.ok) throw new Error(`Failed ${res.status} for ${url}`);
        const blob = await res.blob();
        zip.file(`${String(i + 1).padStart(2, "0")}-${name}`, blob);
        await deleteImage(upscaledFiles?.[i]?.fileId ?? "");
      } catch (e) {
    ...
    saveAs(content, "upscaled-images.zip");
