...
import { resetRemoveBackground } from "~/redux/slices/removeBackgroundSlice";
...
const featureConfig: Record<string, { action: string }> = {
  ...
  "remove-background": { action: "background removed" },
  "crop-image": { action: "cropped" },
  "upscale-image": { action: "upscaled" },
};
...
const DownloadPage = () => {
  ...
  const { removedFiles } = useAppSelector((s) => s.removeBackground);
  const handleDownloadFile = async () => {
    if (!removedFiles?.length) return;

    if (removedFiles.length === 1) {
      const url = removedFiles?.[0]?.removedUrl ?? "";
      const name = filenameFromUrl(url, 0, { replaceTokenWith: "removed" });
      try {
        await downloadOne(url, name);
        await deleteImage(removedFiles?.[0]?.fileId ?? "");
      } catch (e) {
    ...
    for (let i = 0; i < removedFiles.length; i++) {
      const url = removedFiles?.[i]?.removedUrl ?? "";
      const name = filenameFromUrl(url, i, { replaceTokenWith: "removed" });
      try {
        const res = await fetch(url, { credentials: "omit" });
        if (!res.ok) throw new Error(`Failed ${res.status} for ${url}`);
        const blob = await res.blob();
        zip.file(`${String(i + 1).padStart(2, "0")}-${name}`, blob);
        await deleteImage(removedFiles?.[i]?.fileId ?? "");
      } catch (e) {
    ...
    saveAs(content, "removed-images.zip");

    dispatch(resetRemoveBackground());
  };
