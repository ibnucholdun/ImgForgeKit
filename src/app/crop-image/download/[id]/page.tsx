...
const DownloadPage = () => {
  ...
  const { cropedFiles } = useAppSelector((s) => s.crop);
  const handleDownloadFile = async () => {
    if (!cropedFiles?.length) return;

    if (cropedFiles.length === 1) {
      const url = cropedFiles?.[0]?.cropedUrl ?? "";
      const name = filenameFromUrl(url, 0, { replaceTokenWith: "resized" });
      try {
        await downloadOne(url, name);
        await deleteImage(cropedFiles?.[0]?.fileId ?? "");
      } catch (e) {
    ...
    for (let i = 0; i < cropedFiles.length; i++) {
      const url = cropedFiles?.[i]?.cropedUrl ?? "";
      const name = filenameFromUrl(url, i, { replaceTokenWith: "resized" });
      try {
        const res = await fetch(url, { credentials: "omit" });
        if (!res.ok) throw new Error(`Failed ${res.status} for ${url}`);
        const blob = await res.blob();
        zip.file(`${String(i + 1).padStart(2, "0")}-${name}`, blob);
        await deleteImage(cropedFiles?.[i]?.fileId ?? "");
      } catch (e) {
