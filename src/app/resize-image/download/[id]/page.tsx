...
const DownloadPage = () => {
  ...
  const { resizedFiles } = useAppSelector((s) => s.resize);
  const handleDownloadFile = async () => {
    if (!resizedFiles?.length) return;

    if (resizedFiles.length === 1) {
      const url = resizedFiles?.[0]?.resizedUrl ?? "";
      const name = filenameFromUrl(url, 0, { replaceTokenWith: "resized" });
      try {
        await downloadOne(url, name);
        await deleteImage(resizedFiles?.[0]?.fileId ?? "");
      } catch (e) {
    ...
    for (let i = 0; i < resizedFiles.length; i++) {
      const url = resizedFiles?.[i]?.resizedUrl ?? "";
      ...
      try {
        ...
        await deleteImage(resizedFiles?.[i]?.fileId ?? "");
      } catch (e) {
